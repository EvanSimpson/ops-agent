...
func WaitForMetric(ctx context.Context, logger *log.Logger, vm *VM, metric string, window time.Duration, extraFilters []string) (error, *monitoringpb.TimeSeries) {
	for attempt := 1; attempt <= QueryMaxAttempts; attempt++ {
		...
		if err != nil && !isRetriableLookupMetricError(err) {
			return fmt.Errorf("WaitForMetric(metric=%q, extraFilters=%v): %v", metric, extraFilters, err), nil
		}
	...
	return fmt.Errorf("WaitForMetric(metric=%s, extraFilters=%v) failed: exhausted retries", metric, extraFilters), nil
}
